...
import { AlertCircle, ArrowLeft, Calendar, CheckCircle2, DollarSign, FileText, Upload, X } from 'lucide-react';
import { Link } from 'react-router-dom';
import { cn } from '../lib/utils';
import { useAuth } from '../contexts/AuthContext';
import api from '../lib/api';

const PurchaserPopUpload = () => {
  const { user } = useAuth();
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Failed to submit proof of payment. Please try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex flex-col items-center justify-center min-h-[500px] gap-6 animate-in">
        ...
        <div className="text-center">
          ...
          <p className="text-secondary-400 mt-2 max-w-sm mx-auto">
            Your proof of payment has been queued for review. You will see it reflected after verification.
          </p>
        </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-6xl mx-auto">
      <div className="flex flex-col gap-8">
        ...
        <div>
          ...
          <p className="text-secondary-400 mt-2 text-lg italic">Upload a clear receipt or transfer confirmation so your payment can be verified quickly.</p>
        </div>
        ...
        <div className="glass p-8 rounded-[2rem] border-primary-500/10 bg-primary-600/5">
          ...
          <ul className="flex flex-col gap-3 text-sm text-secondary-400">
            <li>Ensure the bank reference number is clearly visible.</li>
            <li>Accepted file formats depend on your file upload setup.</li>
            <li>Proof submissions are reviewed by administration before they affect your statement.</li>
          </ul>
        </div>
      </div>
      ...
      <div className="glass p-8 md:p-10 rounded-[2.5rem] border-white/10 shadow-2xl animate-in">
        <form onSubmit={handleSubmit} className="flex flex-col gap-8">
              ...
              {file ? (
                <div className="flex flex-col items-center gap-2">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-[10px] text-secondary-500 font-black uppercase tracking-widest mt-1">Ready to upload</p>
                  </div>
                  <button
                    type="button"
                    onClick={(event) => {
                ...
                <>
                  <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center text-secondary-400">
                    <Upload size={32} />
                  </div>
                  <div className="text-center">
          ...
          >
            {loading ? <div className="w-6 h-6 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : 'Upload & Notify Admin'}
          </button>
        </form>
      </div>
    </div>
...
export default PurchaserPopUpload;
