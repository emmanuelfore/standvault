...
const PurchaserDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.error('Failed to fetch purchaser dashboard data', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!data) {
    return (
      <div className="glass rounded-3xl p-10 text-center text-secondary-400">
        We could not load your purchaser account summary right now.
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-secondary-400 mt-2 text-lg">
            You have paid <span className="text-primary-400 font-bold">{data.progress_pct}%</span> of your total property contract.
          </p>
        </div>
        <div
          className={cn(
            'px-6 py-3 rounded-2xl border flex items-center gap-3 font-black tracking-widest text-xs uppercase',
            data.balance_status === 'HEALTHY'
          ...
          <div className="relative z-10 flex flex-col gap-8">
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-end justify-between">
                <div className="flex flex-col">
                  <span className="text-5xl font-black">${Number(data.total_paid).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  <span className="text-secondary-500 font-medium mt-1">
                    Paid out of ${Number(data.total_price).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 pt-8 border-t border-white/10">
              ...
              <div>
                ...
                <p className="font-bold text-lg">${Number(data.outstanding).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
              </div>
              <div className="col-span-2 bg-white/5 rounded-2xl p-4 flex items-center justify-between border border-white/5 ring-1 ring-white/10">
                <div>
        ...
        <div className="flex flex-col gap-8">
          <div className="glass p-8 rounded-[2rem] flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-3">
              <Link
                to="/account/proof-of-payment"
                className="flex items-center justify-between p-4 bg-primary-600 hover:bg-primary-700 text-white rounded-2xl font-black tracking-tight transition-all active:scale-95 shadow-xl shadow-primary-600/20 group"
              >
                <span>Submit Proof Of Payment</span>
                <ArrowUpRight size={20} className="group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
              </Link>
              <Link className="flex items-center gap-4 p-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl font-bold transition-all text-sm group" to="/account/payments">
                <div className="w-8 h-8 rounded-lg bg-orange-500/10 text-orange-400 flex items-center justify-center">
                  <CreditCard size={18} />
                </div>
                View Statement
              </Link>
              <Link className="flex items-center gap-4 p-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl font-bold transition-all text-sm group" to="/account/documents">
          ...
          <div className="glass p-8 rounded-[2rem] flex-1">
            ...
            <div className="flex gap-4 p-4 bg-secondary-900/50 rounded-2xl border border-white/5">
              ...
              <div>
                ...
                <p className="text-xs text-secondary-500 mt-1">Your verified payments and charges are recorded against your purchaser account.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center justify-between">
          ...
          <Link to="/account/payments" className="text-primary-500 text-sm font-bold hover:underline">
            ...
            <div key={activity.id} className="glass p-6 rounded-3xl flex flex-col gap-4 glass-hover hover:-translate-y-1 transition-all">
              <div className="flex items-start justify-between">
                <div
                  className={cn(
                    ...
                    activity.entry_type === 'PAYMENT'
                      ? 'bg-green-500/10 text-green-400'
                      : activity.entry_type === 'CHARGE'
                        ? 'bg-orange-500/10 text-orange-400'
                        : 'bg-purple-500/10 text-purple-400'
                  )}
                >
                  {activity.entry_type === 'PAYMENT' ? <CreditCard size={24} /> : activity.entry_type === 'CHARGE' ? <Clock size={24} /> : <TrendingUp size={24} />}
                </div>
              ...
              {activity.amount && (
                <div className="mt-2 pt-4 border-t border-white/5 flex justify-between items-baseline">
                  ...
                  >
                    {activity.entry_type === 'PAYMENT' ? '+' : '-'}${Number(activity.amount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                </div>
              )}
            </div>
