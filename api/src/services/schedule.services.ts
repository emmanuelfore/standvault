...
export class ScheduleService {
  private static async findPrimarySchedule(
    buyerId: string,
    ...
    const activeSchedule = await tx.instalment_schedules.findFirst({
      where: { buyer_id: buyerId, status: ScheduleStatus.ACTIVE },
  ...
  private static isDepositPayment(entry: { entry_type: string; description: string | null | undefined }) {
    return (
  ...
  static calculateScheduleData(config: any, stand: any, options: { 
    startDate?: Date, 
    customInstalments?: number, 
    customDepositAmount?: Prisma.Decimal 
  } = {}) {
  ...
  private static allocatePeriodsFromLedger(periods: any[], ledgerEntries: any[]) {
  ...
  static async materializeSchedule(schedule: any, buyerId: string) {
    if (!schedule) {
      return null;
    }
  ...
  static async generateSchedule(
    ...
    standId: string, 
    options: { startDate?: Date, customInstalments?: number, customDepositAmount?: Prisma.Decimal, depositDate?: Date } = {},
    txContext?: Prisma.TransactionClient
  ) {
    const tx = txContext || prisma;
    
    const buyer = await tx.buyers.findUnique({ where: { id: buyerId } });
    if (!buyer) throw { status: 404, message: `Buyer with ID ${buyerId} not found` };
    ...
    if (!config || !stand) throw new Error('Missing config or stand');
  ...
  static async getSchedule(buyerId: string) {
  ...
  static async getBalance(buyerId: string) {
  ...
  static async getBalanceBreakdown(buyerId: string) {
    const schedule = await this.getSchedule(buyerId);
    ...
    if (schedule && schedule.periods) {
       ...
       const standPrincipal = new Decimal((schedule as any).buyer?.stand?.size_sqm || 0).mul((schedule as any).buyer?.stand?.price_per_sqm || 0);
       const totalInterest = schedule.periods.reduce((acc: Prisma.Decimal, p: any) => acc.add(p.interest_expected || 0), new Decimal(0));
  ...
  static async initializeSchedule(buyerId: string, options: { 
    startDate?: Date, 
    customInstalments?: number, 
    customDepositAmount?: number | Prisma.Decimal 
  } = {}) {
    ...
    const normalizedOptions = {
        ...
        customDepositAmount: options.customDepositAmount !== undefined && typeof options.customDepositAmount === 'number' 
            ...
            : options.customDepositAmount as Prisma.Decimal | undefined
    };
  ...
  static async recalculateAllocations(buyerId: string, txContext?: Prisma.TransactionClient) {
    const tx = txContext || prisma;
    ...
    const standPrincipal = new Decimal((schedule as any).buyer?.stand?.size_sqm || 0).mul((schedule as any).buyer?.stand?.price_per_sqm || 0);
    const config = (schedule as any).buyer?.config || {};
    ...
    let buyersPaymentStatus: 'CURRENT' | 'IN_ARREARS' | 'FULLY_PAID' = 'CURRENT';
