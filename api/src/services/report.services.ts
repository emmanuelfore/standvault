...
export class ReportService {
  private static async getNotice(projectId: string) {
    const closed = await prisma.reconciliation_periods.findFirst({
       where: { project_id: projectId, is_closed: false }
    });
    return closed ? 'PROVISIONAL DATA: An open reconciliation period exists.' : null;
  }

  static async generateReport(type: string, projectId: string, format: string, res: any, query: any = {}) {
    ...
    if (type === 'defaulters') {
       ...
       const buyers = await prisma.buyers.findMany({
        ...
        for (const b of buyers) {
          const breakdown = await ScheduleService.getBalance(b.id);
       ...
       for (const b of buyers) {
          const bd = await ScheduleService.getBalanceBreakdown(b.id);
          rows.push([
            b.id, 
       ...
       const totalStands = await prisma.stands.count({ where: { project_id: projectId } });
       const soldStands = await prisma.stands.count({ where: { project_id: projectId, status: 'SOLD' } });
       const collectedAgg = await prisma.ledger_entries.aggregate({
       ...
       for (let i = 0; i < 12; i++) {
          ...
          const mAgg = await prisma.instalment_periods.aggregate({
             where: {
    ...
    if (format === 'csv') {
       ...
       const output = stringify([ [notice || ''] , headers, ...rows]);
       res.send(output);
    } else if (format === 'xlsx') {
  ...
  static async generateCustomReport(projectId: string, config: any, format: string, res: any) {
    const notice = await this.getNotice(projectId);
    ...
    const types: any[] = [];
    if (config.includePayments === 'true' || config.includePayments === true) types.push('PAYMENT');
    ...
    if (config.includeArrears === 'true' || config.includeArrears === true) {
       ...
       for (const b of buyers) {
          const bd = await ScheduleService.getBalance(b.id);
