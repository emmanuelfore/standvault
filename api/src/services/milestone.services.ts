...
export class MilestoneService {
  static async evaluatePaymentMilestones(buyerId: string) {
    const buyer = await prisma.buyers.findUnique({
       where: { id: buyerId },
       include: { config: true, stand: true }
    });

    if (!buyer || !buyer.config || !buyer.stand) return;

    const breakdown = await ScheduleService.getBalanceBreakdown(buyerId);
    if (!breakdown || breakdown.original_contract_value.equals(0)) return;
    ...
    const currentMilestones = await prisma.milestones.findMany({
    ...
    for (const threshold of thresholds) {
       if (threshold.condition && !existingTypes.has(threshold.type)) {
          ...
          if (threshold.type === '100_PERCENT_PAID') {
             const admins = await prisma.project_admin_assignments.findMany({
  ...
  static async getMilestones(buyerId: string) {
     return await prisma.milestones.findMany({
        where: { buyer_id: buyerId },
