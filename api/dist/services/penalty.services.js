...
class PenaltyService {
    static async calculatePenalty(buyerId) {
        const schedule = await schedule_services_1.ScheduleService.getSchedule(buyerId);
        if (!schedule)
            return null;
        ...
        if (!buyer || !buyer.config)
            return null;
        const balance = await schedule_services_1.ScheduleService.getBalance(buyerId);
        const now = new Date();
        ...
        const calculation = await db_1.prisma.penalty_calculations.create({
            data: {
                buyer_id: buyerId,
    ...
    static async approvePenalty(calculationId, userId) {
