...
class AuditService {
    static async logChange(userId, action, entityType, entityId, details, ipAddress = null) {
        try {
            return await db_1.prisma.audit_log.create({
                data: {
                    user_id: userId,
                    action,
    ...
    static async getLogs(projectId, filters) {
        ...
        return await db_1.prisma.audit_log.findMany({
            where,
